...
/* =========================================================================
   CONFIG — point this at your FastAPI server (see main.py / CLAUDE.md)
   ========================================================================= */
const API_BASE = "http://localhost:8000"; // change to your deployed engine URL
const POLL_INTERVAL_MS = 4000; // INSTRUCTIONS.md says poll every 3–5s

/* -------------------------------------------------------------------------
   Sample answer key — matches the exact shape POST /grade expects
   (see "Answer Key Format" in INSTRUCTIONS.md). Replace with a real
   answer-key editor / upload in your platform; this is just a working default
   so the demo is runnable end to end.
   ------------------------------------------------------------------------- */
...
/* Circular score ring — driven by real percentage from the API */
...
/* -------------------------------------------------------------------------
   ...
   Your API only ever reports "processing" while a job runs — it doesn't
   expose sub-stage granularity. So this stepper advances on a timer purely
   to give the student visual feedback while we poll; it snaps straight to
   "Completed" the moment the real status flips to graded/review_needed.
   ------------------------------------------------------------------------- */
...
export default function GradingDashboard() {
  ...
  const [result, setResult] = useState(null); // full GET /result/{job_id} payload
  ...
  const startGrading = async () => {
    ...
    // fake step advancement just for UX feedback (see ProgressStepper note above)
    ...
    try {
      ...
      const data = await res.json(); // { job_id, status: "processing", submitted_at }
      ...
      setErrorMsg(
        err.message === "Failed to fetch"
          ? `Couldn't reach the grading engine at ${API_BASE}. Is the FastAPI server running?`
          : err.message
      );
    }
  };

  /* ---- GET /result/{job_id}, polled every 3-5s ----------------------- */
  ...
  /* ---- Derive UI-only groupings from breakdown[] --------------------
     Your API doesn't return "strengths" / "weak topics" / "improvement
     plan" fields — those were only in the mock screenshot. We derive
     reasonable equivalents from the real breakdown[] array instead of
     inventing data. ---------------------------------------------------- */
        ...
        {isBusy && (
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
            ...
            <Card className="xl:col-span-2 flex items-center justify-center text-slate-400 text-sm">
              Polling <code className="mx-1 text-slate-600">GET {API_BASE}/result/{jobId}</code>{" "}
